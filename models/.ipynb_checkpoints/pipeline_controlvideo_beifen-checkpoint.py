...
class ControlVideoToVideoPipeline(DiffusionPipeline):
    r"""
    Pipeline for video-to-video generation using Stable Diffusion with ControlNet guidance.
    ...
    def __init__(
        ...
        if isinstance(controlnet, (list, tuple)):
            controlnet = MultiControlNetModel3D(controlnet) ## 这个可以应对多种control的情况，应该不用管
        ...
        self.vae_scale_factor = 2 ** (len(self.vae.config.block_out_channels) - 1)
        # print(self.scheduler.order) ## 1 虽然还不是很清楚这是什么
    ...
    def __call__(
        ...
        **kwargs,
        ...
        ## frames
        ## negative_prompt
        #### 假如要专门处理人脸视频的话，其实还应该加上一些专门针对人脸的信息。
        #### 若要对推理过程进行修改，其实应该针对每一步进行跟踪，看看哪里能插的进去人脸的信息
        ...
        # 3. Encode input prompt
        ## prompt 和 negative_prompt 就直接给喂进去了，后面也没有用到
        ## 后面关注 prompt_embeds 即可。
        prompt_embeds = self._encode_prompt(
            prompt,
            device,
            num_videos_per_prompt,
        ...
        # print(prompt_embeds.size()) ## torch.Size([2, 77, 768])
        ...
        if isinstance(self.controlnet, ControlNetModel3D):
            images = []
            for i_img in frames:
                # print(type(i_img)) <class 'PIL.Image.Image'>
                i_img = self.prepare_image(
                    image=i_img,
                    width=width,
                    height=height,
                    ...
                    dtype=self.controlnet.dtype,
                    do_classifier_free_guidance=do_classifier_free_guidance,
                ) ## 转成 torch tensor，范围在 0-1 之间
                images.append(i_img)
            frames = torch.stack(images, dim=2)  # b x c x f x h x w 这个b为什么是2？？
            # print(frames.size())
            # exit()
        elif isinstance(self.controlnet, MultiControlNetModel3D):
            images = []
        ...
        # print(type(video_images_[0])) # <class 'torch.Tensor'>
        # print(video_images_[0].size()) # torch.Size([1, 3, 512, 512])
        ...
        ## timesteps是所用到的时间的列表。num_inference_steps相当于是timesteps的长度，也是unet真正调用的次数。
        latent_timestep = timesteps[:1].repeat(batch_size * num_videos_per_prompt) ### 相当于说，要把输入的图像，加噪加到这个程度。
        ...
        # print(latents.size()) ## torch.Size([1, 4, frame_num, H//8, W//8])
        # print(latents.max()) # 大于1 查看一个样例是4.x
        # print(latents.min()) # 小于-1 查看一个样例是-4.x
        
        # 问题来了，这个 latents 的连续性如何呢？？ # 回答：完全看不出连续性。。。下面代码是保存视频看连续性的，真的完全没有连续性。
        # def save_channel_video_(_tensor_, _path_):
        #     ## _tensor_的size应该是（frame_num, H//8, W//8）
        #     _channel_frames_np = _tensor_.unsqueeze(3).expand(-1,-1,-1,3).cpu().numpy() * 0.2 * 127.5 + 127.5
        #     _channel_frames_list = []
        #     for fi_ in range(_channel_frames_np.shape[0]):
        #         _channel_frames_list.append(_channel_frames_np[fi_].astype(np.uint8))
        #     imglist2video(_channel_frames_list, _path_, 15)
        # save_channel_video_(latents[0,1,:,:,:],'test1.mp4')
        # print('save done')
        # exit()
        ...
        # Prepare video indices if performing smoothing
        if len(smooth_steps) > 0:
            video_indices = np.arange(video_length) ## 构造视频帧索引
            zero_indices = video_indices[0::2] ## 选择偶数索引
            one_indices = video_indices[1::2] ## 选择奇数索引
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                print(num_inference_steps - i, smooth_steps, (num_inference_steps - i) in smooth_steps)
